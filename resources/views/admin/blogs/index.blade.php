    ...
    <div class="post fs-6 d-flex flex-column-fluid" id="kt_post">
     ...
     <tr>
      ...
      <span class="symbol-label"
          ...
          style="background-image:url({{ asset('uploads/blogs') }}/{{ $row->image }});"></span>
  </a>
  <!--end::Thumbnail-->
                <div class="ms-5">
                    <!--begin::Title-->
                    <a href="#" class="text-gray-800 text-hover-primary fs-5 fw-bolder mb-1"
                    data-kt-ecommerce-category-filter="category_name" >{{ $row->en_title}}</a>
                    <!--end::Title-->
                </div>
            </div>
        </td>
        ...
        <td class="text-end pe-0" data-order="15">
            <input type="hidden" name="" id=""  data-kt-ecommerce-category-filter="category_id" value="{{$row->id}}" >
            <span class="fw-bolder ms-3">{{ $row->blog_date ??'' }}</span>
        </td>
        <!--end::Qty=-->
        <!--begin::Price=-->
        <td class="text-end pe-0">
            <span class="fw-bolder text-dark">{{ $row->category->en_category ?? '' }}</span>
        </td>
        <!--end::Price=-->
        ...
        <form id="delete_{{$row->id}}" action="{{ route('blogs.destroy', $row->id) }}"  method="POST" style="display: none;">
        @csrf
        @method('DELETE')
