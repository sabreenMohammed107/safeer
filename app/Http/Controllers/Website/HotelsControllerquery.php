...
class HotelsControllerquery extends Controller
{
...
$countries=Country::all();
    ...
    public function fetch(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            ->select('hotels.id as hotel_id', 'hotel_enname', 'hotel_arname',
                'hotel_enoverview', 'hotel_aroverview', 'hotel_stars',
                'hotel_banner', 'hotel_logo', 'hotel_enbrief', 'hotel_arbrief',
                'city_id','zone_id', 'details_enaddress', 'hotels.active', 'country_id', 'en_country', 'room_type_costs.from_date', 'room_type_costs.end_date',
                'ar_country', 'en_city', 'ar_city', 'from_date', 'end_date', 'cost', DB::raw('count(review_text) as totalreviews'),
                DB::raw('min(single_cost) as single_cost'))
        ...
        //->leftJoin("hotel_rooms","hotel_rooms.id","=","room_type_costs.hotel_room_id")
            ...
            ->leftJoin("reviews", "hotels.id", "=", "reviews.hotel_id")
            ->leftJoin("cities", "cities.id", "=", "hotels.city_id")
            ->leftJoin("zones", "zones.id", "=", "hotels.zone_id")
            ->leftJoin("countries", "countries.id", "=", "cities.country_id")
            ->where("hotels.active", "=", 1)
            ->groupBy('hotels.id', 'hotel_enname', 'hotel_arname',
                'hotel_enoverview', 'hotel_aroverview', 'hotel_stars',
            ...
            if ($request->hotel_zone_ids) {
                $RoomCosts->whereIn("zone_id", explode(',', $request->hotel_zone_ids));
            ...
            // $Count = $RoomCosts->count();
            $HotelsRecommended = $RoomCosts->take(6)->reorder()->orderBy("hotel_stars",'desc')->get();

            $HotelsByPrice = $RoomCosts->take(6)->reorder()->orderBy("single_cost",'asc')->get();

            $HotelsByAlpha = $RoomCosts->take(6)->reorder()->orderBy("hotel_enname",'asc')->get();
            \Log::info(['sessssssss',$HotelsRecommended]);
    ...
    public function search(Request $request)
    {
        ...
        //->leftJoin("hotel_rooms","hotel_rooms.id","=","room_type_costs.hotel_room_id")
            ...
            $Count = $RoomCosts->count();
            $HotelsRecommended = $RoomCosts->skip(($request->page_num - 1) * 6)->take(6)->orderBy("hotel_stars",'desc')->get();
            $HotelsByPrice = $RoomCosts->skip(($request->page_num - 1) * 6)->take(6)->orderBy("single_cost")->get();


        $HotelsByAlpha = $RoomCosts->skip(($request->page_num - 1) * 6)->take(6)->orderBy("hotels.hotel_enname")->get();
